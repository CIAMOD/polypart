...
def test_requires_enough_halfspaces():
    ...
    with pytest.raises(ValueError):
        get_random_polytope(d=d, m=m)


def test_returns_polytope_with_vertices():
    d = 3
    m = 10
    P = get_random_polytope(d=d, m=m, seed=123)
...
def test_seed_reproducibility():
    d = 4
    m = 20
    P1 = get_random_polytope(d=d, m=m, seed=42)
    P2 = get_random_polytope(d=d, m=m, seed=42)
...
def test_radius_changes_halfspace_offsets_not_normals():
    ...
    P1 = get_random_polytope(d=d, m=m, radius=1.0, seed=seed)
    P2 = get_random_polytope(d=d, m=m, radius=2.0, seed=seed)
