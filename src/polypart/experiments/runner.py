"""Experiment runner for benchmarking algorithms."""
...
class NoDaemonProcess(multiprocessing.Process):
    """Process that can spawn children (non-daemonic)."""
...
def _run_single_algorithm_isolated(args):
    ...
    num_regions = None
    if algo_name == "ppart":
        T, num_regions = build_partition_tree(A, P, "v-entropy", record_stats=True)
    elif algo_name == "incenu":
        _, num_regions = build_incenu_tree(A, [] if bbox_mode else P)
...
def run_experiments(
    ...
    # Parent process count (at least 1 to run the experiment)
    pool_size = max(1, max_workers // (1 + n_algs))  # Parent + n_algs children
